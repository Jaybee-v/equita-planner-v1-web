...
import { findByStableIdAndWeek } from "../services/find-by-stable-id-and-week";
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (weekDates.includes(selectedDate)) {
        ...
        const response = await findByStableIdAndWeek(stable.id, selectedDate);
        setActivities(response as Activity[]);
      }
      setWeek(weekDates);
    };
    fetchData();
  }, [date, stable.id, thisWeekActivities, selectedDate]);
